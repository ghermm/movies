...
function getSearchedMovies(filter: string): Movie[] {
...
function search(filtered: Movie[]) {
    filtered.forEach(movie => {
        document.getElementById("movies")!.insertAdjacentHTML("beforeend", `<li class='movie'></li>`)
        const element = (document.getElementsByClassName("movie")[document.getElementsByClassName("movie").length - 1]) as HTMLElement;
        const imageUrl = getImage(movie.name);
        const img = new Image();
        ...
        element.addEventListener('click', () => playMovie(movie.link));
    })
}

function playMovie(movieLink: string) {
    ...
    (document.getElementById("movie") as HTMLIFrameElement).src = movieLink;
}
...
function simpleMovie(movieName: string, movieLink: string) {
    addMovie({
        description: "",
        genres: [""],
        link: movieLink,
